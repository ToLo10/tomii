...
## 2026-09-15 — Fast and resumable video delivery
- لم يعد إكمال الفيديو ينتظر إغلاق GridFS قبل إرسال الرسالة؛ تُخدم نسخة محلية مطابقة فور وصول آخر بايت، ثم تُثبت النسخة الدائمة بالخلفية مع إعادة المحاولة.
- إذا انقطع اتصال الهاتف بعد قبول مقطع، يعاد استخدام جلسة الرفع نفسها ويُستأنف من آخر إزاحة مؤكدة بدل إعادة الفيديو من البداية.
- صار حجم مقطع الرفع الافتراضي 8MB لتقليل عدد طلبات HTTP، مع دعم حاويات فيديو إضافية وحفظ MIME مناسب للملف الأصلي.

## 2026-09-15 — Desktop video compatibility
- أضيف فحص `ffprobe` بعد الرفع لاكتشاف فيديوهات HEVC/H.265 والحاويات التي لا يشغلها متصفح اللابتوب بصورة صحيحة.
- تُنشأ نسخة H.264/AAC بصيغة MP4 في طابور خلفي محدود، مع إبقاء الملف الأصلي وعدم تأخير رد الرفع.
- رابط المشاهدة يختار النسخة المتوافقة تلقائياً عند اكتمالها، وتصل حالة التجهيز مباشرة للصفحة المفتوحة عبر Socket.IO.
- أزيل الاعتماد على `source.type` القديم من الواجهة حتى لا يحوّل MIME خاطئ من بعض أجهزة Android الفيديو إلى صوت فقط.
